...
//var dao = function() {
	...
	var searchUserByWord = function(word, callback) {
		//var myUser = null;
		console.log('looking for user with word: ' + word);
		var regex = eval('/' + word + '/i');
		User.find({username : regex}).limit(10).exec(function(err, users) {
    		if (err) {
        		console.log(err);
    		}
    		else if (users) {
    			console.log('got' + users.length + 'user');
    			//myUser = user;
    		}
    		else {
...
var newFriendReq = function(friendReq, callback) {
	console.log('adding new friend req');
	...
	checkFriendStatus(from, to, function(err, result) {
		if (err) {
			...
			if (result == 0) {
				FriendReq.findOne({from : from, to : to}, function(err, req) {
					if (err) {
						callback(err, -1);
						return;
					}
					...
					var fr = new FriendReq(friendReq);
					console.log(fr.from);
					console.log(fr.to);
					console.log(fr.msg);
					fr.save(function(err) {
						if (err) {
							console.log('friend req save error');
						}
						else {
							console.log('new friend req saved');
						}
						callback(err, 1);
					});
				});
...
var getFriendReqs = function(username, callback) {
	//var friendReqs = [];
	FriendReq.find({to : username}, function(err, data) {
		if (err) {
			console.log(err);
		}
		else {
			
			console.log(username + ' has ' + data.length + ' requests');
		}
		if (typeof(callback) == 'function')
			callback(err, data);
	});
...
var getFriendList = function(username, callback) {
	Friend.find().or([{from : username},{to : username}]).exec(function(err, data) {
		var friendList = [];
		if (err) {
			console.log(err);
		}
			...
			if (data) {
				for (var i = 0; i < data.length; i++) {
					...
					//console.log(pair);
					if (pair.to == username)
						friendList.push({username : pair.from});
					else
						friendList.push({username : pair.to});
				}
			}
			//friendList = friendList.sort();
			console.log(username + ' has ' + data.length + ' friends');
		}
		if (typeof(callback) == 'function')
...
var deleteMessagesByUser = function(from, to, callback) {
	console.log('delete message by user');
...
module.exports.user.searchByWord = searchUserByWord;
...
module.exports.friend.request = newFriendReq;
